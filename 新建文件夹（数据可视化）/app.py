...
def teacher_dashboard():
    ...
    if request.method == 'POST':
        ...
        new_scaffold = request.form.get('task_scaffold', '') # 💡 获取支架文本
        
        # 🌟🌟 新增 RAG 模块：处理老师上传的自定义评价标准 🌟🌟
        ...
        if clear_rubric == 'yes':
            # 1. 如果老师明确勾选了卸载，那就什么都不读，让 custom_standard_text 保持为空 ""
            pass 
        else:
            # 2. 如果没有勾选卸载，走正常的文件上传和兜底保留逻辑
            ...
            if uploaded_file and uploaded_file.filename != '':
                try:
                    ...
                    return jsonify({"status": "error", "message": "文件读取失败，请确保上传的是 UTF-8 编码的纯文本文件！"}), 400
            else:
                # 如果老师这次没有上传新文件，我们要去看看以前是不是已经传过了，如果有就保留！
                old_task = get_current_task()
                custom_standard_text = old_task.get('custom_standard', '')

        if new_title and new_content:
            try:
                # 💡 注意：这里传了 4 个参数给保存函数！(如果勾选了清除，传进去的就是空文本)
                save_current_task(new_title, new_content, new_scaffold, custom_standard_text)
                
                # 直接返回 JSON 给前端的 Fetch API（触发按钮变绿）
                from flask import jsonify
