...
with app.app_context():
    ...
    if not User.query.first():
        if os.path.exists(USER_DB):
            ...
            db.session.add(User(username='admin', password='123456', role='teacher', real_name='李老师'))
            db.session.add(User(username='2026001', password='123456', role='student', real_name='张三'))
            db.session.commit()
...
def verify_login(username, password):
    ...
    if user:
        return {'username': user.username, 'real_name': user.real_name, 'role': user.role}
    return None
