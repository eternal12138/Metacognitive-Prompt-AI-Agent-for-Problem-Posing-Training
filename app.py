...
class MetacognitiveState(db.Model):
    """每名学生在每个任务中的元认知提示状态。"""
    __bind_key__ = "business_db"
    __table_args__ = (
        db.UniqueConstraint("student_id", "task_name", name="uq_metacognitive_student_task"),
    )

    id = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.String(50), nullable=False, index=True)
    task_name = db.Column(db.String(200), nullable=False, index=True)
    phase = db.Column(db.String(20), nullable=False, default="planning")
    turn_count = db.Column(db.Integer, nullable=False, default=0)
    prompt_count = db.Column(db.Integer, nullable=False, default=0)
    weak_input_streak = db.Column(db.Integer, nullable=False, default=0)
    last_question_normalized = db.Column(db.Text, default="")
    last_prompt_type = db.Column(db.String(20), default="")
...
class PromptEvent(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    chat_record_id = db.Column(db.Integer, unique=True, nullable=False, index=True)
    event_time = db.Column(db.String(50), nullable=False)
    student_id = db.Column(db.String(50), nullable=False, index=True)
    task_name = db.Column(db.String(200), nullable=False, index=True)
    cycle_index = db.Column(db.Integer, nullable=False, default=1)
    turn_index = db.Column(db.Integer, nullable=False)
    phase_before = db.Column(db.String(20), nullable=False)
    phase_after = db.Column(db.String(20), nullable=False)
    prompt_type = db.Column(db.String(20), nullable=False, index=True)
    trigger_reason = db.Column(db.String(100), nullable=False)
    support_level = db.Column(db.String(20), nullable=False, default="medium")
    ...
    prompt_version = db.Column(db.String(50), nullable=False, default="mp-sm-v1")
...
PROMPT_TRIGGER_LABELS = {
    "task_cycle_started": "新探究周期开始",
    "ongoing_self_monitoring": "持续检查提问过程",
    "repeated_question": "问题内容重复或高度相似",
    "weak_or_stalled_input": "输入过短或遇到困难",
    "student_requested_reflection": "学生主动结束本轮并反思",
}

METACOGNITIVE_INSTRUCTIONS = {
    "planning": """
【本轮元认知策略：计划提示】
这是该学生本轮任务的首次交互。先帮助学生明确“准备探究什么、材料中哪些信息值得关注、准备从哪个方向提出问题”。
不要替学生生成完整问题；用 1–2 个简短问题促使学生说出目标、已知条件或提问方向。
""",
    "monitoring": """
【本轮元认知策略：监控提示】
帮助学生检查自己的理解和提问过程。引导其判断：当前问题是否紧扣材料、条件是否充分、依据是什么、还有哪个概念不确定。
只选择最相关的一个监控点追问，不要一次列出检查清单，也不要直接改写成标准答案。
""",
    "regulating": """
【本轮元认知策略：调节提示】
学生当前可能停滞、输入过短或重复。提供一个可以立即执行的小台阶，例如定位材料细节、补充一个条件、选择一个变量或换一个观察角度。
支架要具体但保持最小干预；不能直接给出完整问题或科学结论。回复只包含 1 个主要行动建议和至多 1 个追问。
""",
    "reflecting": """
【本轮元认知策略：反思提示】
引导学生回顾本轮提问过程：最初关注什么、后来增加了哪些条件或角度、当前问题比最初好在哪里、下一次还可怎样改进。
可以简要引用其历史提问特征，但不要代替学生下结论；用 1–2 个问题促使学生完成自我评价。
""",
}
...
def save_current_task(title, content, scaffold="", custom_standard=""):
    ...
    task_changed = (
        task.title != title
        or task.content != content
        or task.scaffold != scaffold
        or task.custom_standard != custom_standard
    )
    task.title = title
    ...
    if task_changed:
        MetacognitiveState.query.filter(
            MetacognitiveState.task_name.in_({previous_title, title})
...
def _is_weak_input(text):
    normalized = _normalize_question(text)
    stalled_markers = ("不知道", "不会", "不懂", "没想法", "想不到")
    short_weak_phrases = {"为什么", "怎么问", "怎么办", "不知道问什么"}
    return (
        len(normalized) <= 4
        or normalized in short_weak_phrases
        or any(marker in normalized for marker in stalled_markers)
    )


def get_or_create_metacognitive_state(student_id, task_name):
    state = MetacognitiveState.query.filter_by(
    ...
    existing_records = ChatRecord.query.filter_by(
        student_id=student_id,
        task_name=task_name,
    ).order_by(ChatRecord.id.asc()).all()
    existing_turns = len(existing_records)
    last_question = existing_records[-1].question if existing_records else ""
    state = MetacognitiveState(
        student_id=student_id,
        task_name=task_name,
        phase="monitoring" if existing_turns else "planning",
        turn_count=existing_turns,
        prompt_count=0,
        weak_input_streak=0,
        last_question_normalized=_normalize_question(last_question),
        cycle_index=1,
        started_at=existing_records[0].time if existing_records else now,
        updated_at=now,
    )
...
def decide_metacognitive_prompt(state, student_input, action="chat"):
    """Purely rule-driven transition so each intervention is reproducible."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    normalized = _normalize_question(student_input)

    if state.completed_at and action != "reflect":
        state.phase = "planning"
        state.turn_count = 0
        state.prompt_count = 0
        state.weak_input_streak = 0
    ...
    similarity = (
        SequenceMatcher(None, normalized, state.last_question_normalized).ratio()
        if normalized and state.last_question_normalized
        else 0.0
    )
    repeated = bool(normalized) and (
        normalized == state.last_question_normalized
        or (len(normalized) >= 8 and similarity >= 0.88)
    )
    weak_input = _is_weak_input(student_input)
    explicit_reflection = action == "reflect" or any(
        marker in student_input
        for marker in ("结束本轮", "开始反思", "回顾我的提问", "总结我的提问过程")
    )

    if explicit_reflection:
        prompt_type = "reflecting"
        trigger_reason = "student_requested_reflection"
    elif state.turn_count == 0:
        prompt_type = "planning"
        trigger_reason = "task_cycle_started"
    elif repeated:
        prompt_type = "regulating"
        trigger_reason = "repeated_question"
    elif weak_input:
        prompt_type = "regulating"
        trigger_reason = "weak_or_stalled_input"
    else:
        prompt_type = "monitoring"
        trigger_reason = "ongoing_self_monitoring"

    if prompt_type == "regulating" and state.weak_input_streak >= 1:
        support_level = "high"
    elif prompt_type == "monitoring" and state.turn_count >= 4:
        support_level = "low"
    else:
        support_level = "medium"

    if weak_input or repeated:
        state.weak_input_streak += 1
    else:
        state.weak_input_streak = 0

    state.phase = prompt_type
    state.turn_count += 1
    state.prompt_count += 1
    state.last_question_normalized = normalized
    state.last_prompt_type = prompt_type
    state.updated_at = now
    if prompt_type == "reflecting":
        state.completed_at = now

    return {
        "phase_before": phase_before,
        "phase_after": prompt_type,
        "prompt_type": prompt_type,
        "prompt_label": PROMPT_TYPE_LABELS[prompt_type],
        "trigger_reason": trigger_reason,
        "support_level": support_level,
        "instruction": METACOGNITIVE_INSTRUCTIONS[prompt_type] + (
            "\n【支架强度：低】学生已有多轮探索，只做轻量追问，优先让学生自己决定下一步。\n"
            if support_level == "low"
            else "\n【支架强度：高】学生连续遇到困难，请给出更具体但仍不泄露答案的单一步骤。\n"
            if support_level == "high"
            else "\n【支架强度：中】提供适度方向提示，并保留学生自主选择空间。\n"
        ),
...
def student_chat():
    ...
    current_state = MetacognitiveState.query.filter_by(
        student_id=student_id,
        task_name=current_task.get("title"),
    ).first()
    ...
    return render_template('student.html',
                           ...
                           metacognitive_state={
                               "phase": current_state.phase if current_state else "planning",
                               "phase_label": PROMPT_TYPE_LABELS.get(
                                   current_state.phase if current_state else "planning",
                                   "计划性提示",
                               ),
                               "turn_count": current_state.turn_count if current_state else 0,
                           })
...
def chat():
    ...
    if action not in {"chat", "reflect"}:
        return jsonify({"error": "未知的学习操作"}), 400
    if action == "reflect" and not user_input:
        user_input = "我想结束本轮探究，请引导我回顾自己的提问过程。"
    if not user_input:
    ...
    metacognitive_state = get_or_create_metacognitive_state(student_account, task_name)
    metacognitive_decision = decide_metacognitive_prompt(
        metacognitive_state,
        user_input,
        action=action,
    )
...
【最终输出要求（严格遵守）】
    ...
    rag_context = ""
    try:
        question_vector = None if action == "reflect" else get_text_embedding(user_input)
        if question_vector:
    ...
    enhanced_system_prompt = system_prompt + metacognitive_decision["instruction"] + f"""
...
"""
    ...
    prompt_event = PromptEvent(
        ...
        student_id=student_account,
        task_name=task_name,
        cycle_index=metacognitive_decision["cycle_index"],
        turn_index=metacognitive_decision["turn_index"],
        phase_before=metacognitive_decision["phase_before"],
        phase_after=metacognitive_decision["phase_after"],
        prompt_type=metacognitive_decision["prompt_type"],
        trigger_reason=metacognitive_decision["trigger_reason"],
        support_level=metacognitive_decision["support_level"],
        student_input=user_input,
        model_name=CHAT_MODEL_NAME,
        prompt_version="mp-sm-v1",
    )
    db.session.add(prompt_event)
    db.session.commit()
    ...
    def generate_stream():
        ...
        meta_payload = json.dumps(
            {
                "meta": {
                    "prompt_type": metacognitive_decision["prompt_type"],
                    "prompt_label": metacognitive_decision["prompt_label"],
                    "trigger_reason": metacognitive_decision["trigger_reason"],
                    "trigger_label": PROMPT_TRIGGER_LABELS.get(
                        metacognitive_decision["trigger_reason"],
                        metacognitive_decision["trigger_reason"],
                    ),
                    "support_level": metacognitive_decision["support_level"],
                    "turn_index": metacognitive_decision["turn_index"],
        ...
        try:
            ...
            try:
                ...
                event = db.session.get(PromptEvent, prompt_event_id)
                if record:
                    record.ai_response = full_ai_response
                if event:
                    event.prompt_text = full_ai_response
                if record or event:
                    db.session.commit()
                if record and action != "reflect":
                    task_pool.submit(
...
def teacher_dashboard():
    ...
    prompt_events = PromptEvent.query.filter(
        PromptEvent.chat_record_id.in_([record.id for record in db_records])
    ).all() if db_records else []
    event_by_record = {event.chat_record_id: event for event in prompt_events}
    
    # 伪装成前端需要的列表
    records = [
        [
            r.time, r.student_name, r.student_id, r.question, r.ai_response,
            r.teacher_score, r.teacher_feedback, r.task_name, r.task_content,
            event_by_record[r.id].prompt_type if r.id in event_by_record else "",
            event_by_record[r.id].trigger_reason if r.id in event_by_record else "",
            event_by_record[r.id].turn_index if r.id in event_by_record else "",
            event_by_record[r.id].cycle_index if r.id in event_by_record else "",
            event_by_record[r.id].support_level if r.id in event_by_record else "",
        ]
        for r in db_records
    ]
...
def delete_user():
    ...
    if user:
        ...
        try:
            ...
            PromptEvent.query.filter_by(student_id=user.username).delete()
            MetacognitiveState.query.filter_by(student_id=user.username).delete()
...
def export_data():
    ...
    try:
        ...
        prompt_events = PromptEvent.query.filter(
            PromptEvent.chat_record_id.in_([record.id for record in records])
        ).all() if records else []
        event_by_record = {event.chat_record_id: event for event in prompt_events}
        ...
        # 4. 写入表头
        writer.writerow([
            '记录时间', '姓名', '学号', '学生提问', 'AI元认知提示',
            '教师评分', '教师反馈', '所属任务', '任务背景材料',
            '提示类型', '触发原因', '阶段转换前', '阶段转换后',
            '任务轮次', '探究周期', '提示词版本', '模型名称',
            '支架强度',
        ])

        # 5. 遍历数据库对象，写入数据
        for r in records:
            event = event_by_record.get(r.id)
            writer.writerow([
                ...
                _safe_csv_cell(event.prompt_type if event else ""),
                _safe_csv_cell(event.trigger_reason if event else ""),
                _safe_csv_cell(event.phase_before if event else ""),
                _safe_csv_cell(event.phase_after if event else ""),
                _safe_csv_cell(event.turn_index if event else ""),
                _safe_csv_cell(event.cycle_index if event else ""),
                _safe_csv_cell(event.prompt_version if event else ""),
                _safe_csv_cell(event.model_name if event else ""),
                _safe_csv_cell(event.support_level if event else ""),
            ])
