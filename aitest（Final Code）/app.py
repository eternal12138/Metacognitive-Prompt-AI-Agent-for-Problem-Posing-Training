...
API_KEY = "****************" 
API_URL = "****************" 
CHAT_MODEL_NAME = "doubao-seed-2-0-pro-260215" # 问答模型使用 Doubao-Seed-2.0-pro
EVAL_MODEL_NAME = "deepseek-v3-2-251201" # 评价模型使用 Deepseek-V3.2
...
with app.app_context():
    ...
    if not User.query.first():
        if os.path.exists(USER_DB):
            ...
            super_admin = User(username='admin', password=generate_password_hash('123456'), role='teacher', real_name='超管', can_manage_users=True)
...
def save_current_task(title, content, scaffold="", custom_standard="", image_files=None):
    """保存新任务到 SQLite"""
    task = TaskConfig.query.get(1)
    task.title = title
    ...
    if image_files is not None:
        task.image_files = image_files
    db.session.commit()

def allowed_task_image(filename):
    allowed_extensions = {"png", "jpg", "jpeg", "gif", "webp", "bmp"}
    return "." in filename and filename.rsplit(".", 1)[1].lower() in allowed_extensions

def save_task_images(uploaded_files):
    saved_files = []
    for uploaded_file in uploaded_files:
        if not uploaded_file or not uploaded_file.filename:
            continue
        if not allowed_task_image(uploaded_file.filename):
            continue

        original_name = secure_filename(uploaded_file.filename)
        unique_name = f"{uuid.uuid4().hex}_{original_name}"
        file_path = os.path.join(TASK_IMAGE_DIR, unique_name)
...
@app.route('/task-images/<path:filename>')
def task_image_file(filename):
    return send_from_directory(TASK_IMAGE_DIR, filename)
...
def login():
    ...
    # AI辅助生成：deepseek-v3-2-251201，2026-04-01 设备嗅探
...
def get_current_task():
    ...
    if os.path.exists(TASK_FILE):
        with open(TASK_FILE, 'r', encoding='utf-8') as f:
            ...
            task_data.setdefault("image_files", [])
            task_data["content"] = normalize_task_content(task_data.get("content", ""))
            return task_data
    # 默认值加入了 custom_standard
    return {
        "title": "等待老师发布任务...", 
        "content": "请同学们稍安勿躁。", 
        "scaffold": "",
...
def normalize_task_content(content):
    if not content:
        return ""
    normalized = content.replace("<br />", "\n").replace("<br/>", "\n").replace("<br>", "\n")
    return normalized.replace("\r\n", "\n").replace("\r", "\n")

def delete_task_images(image_files):
    for image_file in image_files or []:
        file_path = os.path.join(TASK_IMAGE_DIR, image_file)
...
task_pool = ThreadPoolExecutor(max_workers=20)

# AI辅助生成：deepseek-v3-2-251201，2026-04-25 这个函数负责在后台默默干脏活累活，绝不阻塞前端的聊天体验！
def save_all_to_db_background(student_id, student_name, task_name, task_content_str, user_input, full_ai_response):
    with app.app_context():
        try:
            ...
            save_to_vector_db_background(student_id, task_name, user_input, full_ai_response, record_id)
            print(f"✅ [后台系统] 聊天记录 {record_id} 保存与记忆压缩完毕！")
        except Exception as e:
            print(f"❌ [后台系统] 写入数据库彻底失败: {e}")
            db.session.rollback()
...
"""
    ...
    def generate_stream():
        ...
        # AI辅助生成：deepseek-v3-2-251201，2026-04-25 核心破局点：发完最后的话，立刻扔出暗号！
        yield f"data: {json.dumps({'text': '[DONE_MARKER]'}, ensure_ascii=False)}\n\n"

        # AI辅助生成：deepseek-v3-2-251201，2026-04-25 把所有存数据库、压缩记忆的任务全部丢进后台线程池，主线程直接下班闭环！
        task_name = current_task.get('title', '未分类当前任务')
        task_content_str = current_task.get('content', '')
        task_pool.submit(save_all_to_db_background, student_account, student_name, task_name, task_content_str, user_input, full_ai_response)
...
def teacher_dashboard():
    ...
    if request.method == 'POST':
        ...
        clear_old_images = request.form.get('clear_old_images') == 'yes'
        ...
        image_files = old_task.get('image_files', [])
        new_image_files = save_task_images(uploaded_images)
        if new_image_files:
            delete_task_images(image_files)
            image_files = new_image_files
        elif clear_old_images:
            delete_task_images(image_files)
            image_files = []

        if new_title and new_content:
            try:
                save_current_task(new_title, new_content, new_scaffold, custom_standard_text, image_files)
    ...
    # AI辅助生成：deepseek-v3-2-251201，2026-04-01 核心升级：设备嗅探与路由分离！
    user_agent = request.headers.get('User-Agent', '').lower()
...
def list_users():
    ...
    if curr_user.can_manage_users:
        ...
        # 普通教师看：自己 + 属于他管理班级的学生
        users = User.query.filter((User.role == 'student') | (User.username == curr_user.username)).all()
...
def batch_add_users():
    ...
    data = request.json  # 期望格式: {"users": [{"username": "2101", "real_name": "张三", "role": "student"}, ...]}
    ...
    for u_data in user_list:
        ...
        if User.query.filter_by(username=u_data['username']).first():
            errors.append(f"账号 {u_data['username']} 已存在，跳过")
            continue

        default_hashed_pwd = generate_password_hash("123456")  # 统一设置初始密码
        ...
        new_user = User(
            ...
            class_name=u_data.get('class_name', '') # 可选字段
        )
        db.session.add(new_user)
        success_count += 1
...
def batch_assign_classes():
    ...
    if action == 'assign_students':
        ...
        User.query.filter(User.username.in_(student_ids)).update({User.class_name: target_class}, synchronize_session=False)
        ...
        if teacher:
            teacher.managed_classes = data.get('managed_classes')
...
def admin_reset_password():
    # 鉴权：只有教师或管理员能操作
    if session.get('role') not in ['teacher']:
    ...
    # 教师只能改学生密码的保护逻辑
    if session.get('role') == 'teacher' and not session.get('can_manage_users'):
        if user.role != 'student' and user.username != session.get('username'):
            return jsonify({"message": "教师仅能修改学生或自己的密码"}), 403
...
def export_data():
    ...
    try:
        ...
        records = ChatRecord.query.order_by(
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5000)  #<-- 确保这行前面有 # 号，或者直接删了
