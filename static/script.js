...
async function sendMessage(action = 'chat') {
    const inputField = document.getElementById('user-input');
    const sendBtn = document.getElementById('send-btn');
    const reflectionBtn = document.getElementById('reflection-btn');
    const message = action === 'reflect'
        ? '我想结束本轮探究，请引导我回顾自己的提问过程。'
    ...
    if (reflectionBtn) reflectionBtn.disabled = true;
    ...
    try {
            ...
            for (let line of lines) {
                ...
                if (line.startsWith('data:')) {
                    ...
                    try {
                        ...
                        if (dataObj.meta) {
                            updateMetacognitiveState(dataObj.meta);
                            continue;
                        }
                        if (dataObj.text === '[DONE_MARKER]' || dataObj.text === '[DONE]') {
                            streamFinished = true;
                            break;
                        }
        ...
        if (action !== 'reflect' && typeof updateCognitiveChart === 'function') {
...
function updateMetacognitiveState(meta) {
    const badge = document.getElementById('metacognitive-state-badge');
    if (!badge || !meta) return;
    const label = meta.prompt_label || meta.prompt_type || '元认知提示';
    const turn = Number(meta.turn_index || 0);
    badge.textContent = `当前：${label} · 第 ${turn} 轮`;
    badge.dataset.promptType = meta.prompt_type || '';
    badge.title = meta.trigger_label || meta.trigger_reason || '';
}
// ==========================================
// 3. 认知轨迹图表的异步更新逻辑
// ==========================================
