...
class StaticSecurityTests(unittest.TestCase):
    def test_no_known_hardcoded_secrets(self):
        forbidden = (
            ...
            "example-secret-that-must-not-appear",
    ...
    def test_four_metacognitive_prompt_types_are_implemented(self):
        for prompt_type in ("planning", "monitoring", "regulating", "reflecting"):
            self.assertIn(f'"{prompt_type}"', APP_SOURCE)
        self.assertIn("class MetacognitiveState", APP_SOURCE)
        self.assertIn("class PromptEvent", APP_SOURCE)
